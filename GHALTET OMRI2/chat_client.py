...
from tkinter import END, PhotoImage, Variable, ttk
...
#******************pour ameliorer la resolution de l'ecran****************
try:
    from ctypes import windll
    windll.shcore.SetProcessDpiAwareness(1)
except:
    pass
#*******************Fin resolution de l'ecran******************************
...
class FirstScreen(tk.Tk,threading.Thread):
    def __init__(self):
        ...
        self.geometry(f"1100x650+350+10")
        self.title("Ahkiless")
        ...
        background = background.resize((1100, 650), Image.ANTIALIAS)
        background = ImageTk.PhotoImage(background)

        upload_image = Image.open('images/upload.png')
        upload_image = upload_image.resize((45, 45), Image.ANTIALIAS)
 ...
 #****************************frame & widgets*******************************
        var = tk.StringVar()
        var.set('male')
        my_font = tk.font.Font(self.first_frame, ("arial", 12, "bold italic"))
        countries = []
        ...
        tphoto = Image.open(f"photo.png")
        tphoto = tphoto.resize((45, 45), Image.ANTIALIAS)
        tphoto = ImageTk.PhotoImage(tphoto)
        
        self.take_photo=tk.Label(
            self.right_frame, 
            text="Take a photo",
            cursor='hand2',
            fg="#541C1C",
            bg='#B3BDE6',
            font=my_font
            ).grid(row=6, column=0, sticky=tk.E, pady=10)

        
        self.take_photo_btn = tk.Button(
            self.right_frame, 
            width=55, 
            #text='Take a Photo',
            image=tphoto,
        ...
        self.take_photo_btn.grid(row=6, column=0, sticky=tk.W, pady=10)
        ...
        upload_button.grid(row=6, column=1, sticky=tk.W, pady=10)
 ...
 # *************************end frame & widgetw*****************************       
        ...
        # head = tk.Label(self.first_frame, text="Login/Enregistrement", font="lucida 17 bold", bg="grey")
...
#*************************fonction verif utilisateur exite ou non
    ...
    def insert_record(self):
        ...
        cur.execute("INSERT INTO record VALUES (:name, :email, :contact, :password)", {
...
class ChatScreen(tk.Canvas, threading.Thread):
    def __init__(self, parent, first_frame, client_socket, clients_connected, user_id):
        ...
        global background
        background = Image.open("images/fontchat.png")
        background = background.resize((700, 650), Image.ANTIALIAS)
        background = ImageTk.PhotoImage(background)
        self.first_frame = first_frame
        self.first_frame.pack_forget()
        tk.Label(self, image=background).place(x=0, y=0)
        self.parent = parent
        ...
        global group_photo
        group_photo = Image.open('images/logogris.png')
        ...
        self.create_text(54, 120, text="Online", font="lucida 12 bold", fill="#40C961")

        tk.Label(self, text="   ", font="lucida 15 bold", bg="#b5b3b3").place(x=4, y=29)

        # tk.Label(self, text="AHKILESS", font="lucida 15 bold", padx=20, fg="#ffbd59",
       ...
       # ****************************Menu ******************************
        ...
        menubar.add_command(label="A Propos", command=self.videocall)
        menubar.add_command(label="Reminder", command=self.runTimer)
        menubar.add_command(label="Help", command=self.videocall)
        menubar.add_command(label="Exit!", command=self.parent.destroy)
...
# *******************************Affichage du photo de l'utilisateur********************
    def clients_online(self, new_added):
        ...
        if not new_added:
            ...
            for user_id in self.clients_connected:
                ...
                b = tk.Label(self, image=user, text=name, compound="left",cursor='hand2', fg="white", bg="#2b2b2b", font="lucida 10 bold", padx=15)
                b.image = user
                self.clients_online_labels[user_id] = (b, self.y)
                b.place(x=50, y=self.y)
            ...
            b = tk.Label(self, image=user, text=name, compound="left",cursor='hand2', fg="white", bg="#2b2b2b",
    ...
    def enter(self,event):
        ...
        #print("la fonction a reussit Si+ {name}")
        # emoji_label.place(x=emoji_x_pos, y=emoji_y_pos)
        # send_button.place(x=400, y=680)
        self.create_image(60, 40, image=group_photo)

        container = tk.Frame(self)
        container.place(x=40, y=100, width=450, height=550)
        self.canvas = tk.Canvas(container, bg="#595656")
        self.scrollable_frame = tk.Frame(self.canvas, bg="#595656")
        scrollable_window = self.canvas.create_window((0, 0), window=self.scrollable_frame, anchor="nw")
        def configure_scroll_region(e):
